...
const SCREENER_TOOLS = new Set(["deploy_position", "swap_token", "get_active_bin", "get_top_candidates", "check_smart_wallets_on_pool", "get_token_holders", "get_token_narrative", "get_token_info", "search_pools", "get_pool_memory", "get_wallet_balance", "get_my_positions"]);
...
const INTENT_TOOLS = {
  ...
  deploy:      new Set(["deploy_position", "swap_token", "get_top_candidates", "get_active_bin", "get_pool_memory", "check_smart_wallets_on_pool", "get_token_holders", "get_token_narrative", "get_token_info", "search_pools", "get_wallet_balance", "get_my_positions", "add_pool_note"]),
...
function shouldRequireRealToolUse(goal, agentType, interactive = false) {
  ...
  // Autonomous screening cycles may legitimately decide not to deploy (⛔ NO DEPLOY)
  if (agentType === "SCREENER" && !interactive) return false;
...
export async function agentLoop(goal, maxSteps = config.llm.maxSteps, sessionHistory = [], agentType = "GENERAL", model = null, maxOutputTokens = null, options = {}) {
  ...
  for (let step = 0; step < maxSteps; step++) {
    ...
    try {
      ...
      let toolChoice = (step === 0 && (ACTION_INTENTS.test(goal) || mustUseRealTool) && !isThinkingModel) ? "required" : "auto";

      for (let attempt = 0; attempt < 3; attempt++) {
        const temp = isThinkingModel ? 1 : config.llm.temperature;
        try {
